    ...
    <div className={cn('flex flex-col gap-6', className)} {...props}>
      <Card className='overflow-hidden p-0 border border-border shadow-lg'>
        <CardContent className='grid p-0 md:grid-cols-2'>
          <form action={formAction} className='p-6 md:p-8 flex flex-col justify-center'>
            <FieldGroup>
              <div className='flex flex-col items-center gap-2 text-center mb-4'>
                <h1 className='text-2xl font-bold text-didongviet-red'>Đặt lại mật khẩu</h1>
              ...
              {state.success && (
                <div className='p-3 text-sm bg-green-500/10 border border-green-500/20 text-green-600 rounded-md flex items-center gap-2 font-medium animate-in fade-in-50 duration-200'>
                  ...
                  <span>{state.message || 'Thay đổi mật khẩu thành công! Đang chuyển hướng...'}</span>
                </div>
              )}
              ...
              {/* MÃ OTP */}
              <Field>
                <FieldLabel htmlFor='otpCode'>Mã xác thực OTP (6 số)</FieldLabel>
                <Input
                  id='otpCode'
                  name='otpCode'
                  type='text'
              ...
              <FieldDescription className='text-center mt-4'>
                <a href='/login' className='inline-flex items-center gap-1.5 text-sm underline-offset-4 hover:underline hover:text-didongviet-red transition-all'>
                  <ArrowLeft size={14} />
