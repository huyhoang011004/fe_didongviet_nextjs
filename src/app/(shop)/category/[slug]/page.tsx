    ...
    async function loadCategoryData() {
      try {
        ...
        const [catRes, productsRes] = await Promise.all([
          fetch(`${apiUrl}/categories/slug/${slug}`).then((r) =>
            r.ok ? r.json() : null,
          ),
          fetch(`${apiUrl}/products/category/${slug}`).then((r) =>
            r.ok ? r.json() : null,
          ),
        ]);
        ...
        if (catRes && catRes.success) {
          ...
          setCategory({
            name: catName,
            description: `Khám phá các dòng sản phẩm ${catName} chính hãng.`,
          });
        }
        ...
        if (productsRes && productsRes.success) {
          ...
          const allRes = await fetch(`${apiUrl}/products`).then((r) =>
            r.ok ? r.json() : null,
          );
        ...
        <div className='max-w-6xl mx-auto px-4 relative z-10 space-y-1.5'>
          ...
          <p className='text-[10px] sm:text-xs text-slate-400 font-medium max-w-xl leading-relaxed'>
            {category?.description ||
              `Tuyển tập sản phẩm ${category?.name} chính hãng ưu đãi tại Di Động Việt.`}
          </p>
        </div>
      </section>
          ...
          {filteredProducts.length === 0 ? (
            <div className='text-center py-16 text-gray-400 border border-dashed border-gray-200 rounded-xl bg-white'>
              ...
              <p className='text-xs font-semibold'>
                Chưa có sản phẩm nào thuộc ngành hàng này.
              </p>
            </div>
          ) : (
