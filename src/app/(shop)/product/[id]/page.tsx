...
export default function ProductDetailPage() {
    ...
    async function loadData() {
      try {
        ...
        if (productRes && productRes.success) {
          ...
          const mainThumb = prod.images?.find((img: any) => img.isThumbnail)?.url || prod.imageUrl || '';
          setActiveImage(mainThumb);

          const relatedRes = await fetch(`${apiUrl}/products/${prod._id}/related`).then(r => r.ok ? r.json() : null);
  ...
  const handleAddToCart = async () => {
    ...
    try {
      ...
      if (res.status === 401) {
        setAlert({ type: 'error', message: 'Vui lòng đăng nhập để thêm vào giỏ hàng' });
        setTimeout(() => router.push('/login'), 1500);
        return;
      }

      if (data.success) {
        setAlert({ type: 'success', message: `Đã thêm ${cartQty} sản phẩm vào giỏ hàng!` });
      } else {
        setAlert({ type: 'error', message: data.message || 'Không thể thêm vào giỏ hàng' });
      }
    } catch (error) {
  ...
  if (loading) {
    return (
      <div className='min-h-screen bg-slate-50 flex flex-col items-center justify-center p-8'>
        ...
        <p className='mt-3 text-xs font-medium text-slate-500 animate-pulse'>Đang tải thông tin sản phẩm...</p>
      </div>
    );
  }
  ...
  if (!product) {
    return (
      <div className='min-h-screen bg-slate-50 flex flex-col items-center justify-center p-8 text-center'>
        ...
        <h2 className='text-sm font-black text-slate-800 uppercase'>Không tìm thấy sản phẩm</h2>
        <p className='text-[10px] text-gray-500 max-w-sm mt-1 mb-4 leading-relaxed'>Sản phẩm không tồn tại hoặc đã ngừng kinh doanh.</p>
        <Button onClick={() => router.push('/')} className='bg-didongviet-red hover:bg-didongviet-dark-red text-white py-2.5 px-5 text-xs font-semibold rounded-xl border-none shadow-sm'>
          Quay lại Trang chủ
        </Button>
      </div>
    );
  ...
  const currentVariantStock = activeVariant.inventory?.reduce((sum: number, inv: any) => sum + (inv.stock || 0), 0) || 0;

  const allImages = [...(product.images || [])];
  if (activeVariant.variantImage && !allImages.some((img: any) => img.url === activeVariant.variantImage)) {
    allImages.push({ url: activeVariant.variantImage, alt: activeVariant.color });
  }
    ...
    <div className='min-h-screen bg-slate-50 font-sans text-slate-700'>
      ...
      <nav className='bg-white border-b border-slate-100 py-2.5'>
        <div className='max-w-6xl mx-auto px-4 flex items-center gap-1.5 text-[10px] text-gray-400 font-semibold overflow-x-auto whitespace-nowrap'>
          ...
          <Link href={`/category/${product.category?.slug || 'products'}`} className='hover:text-didongviet-red transition-colors'>
        ...
        <section className='bg-white rounded-2xl p-5 md:p-6 border border-slate-100 shadow-xs grid grid-cols-1 lg:grid-cols-12 gap-6 relative overflow-hidden'>
          ...
          <div className='lg:col-span-5 space-y-3 relative z-10'>
            ...
            <div className='bg-slate-50 border border-slate-100 rounded-xl p-3.5 space-y-2.5 text-[10px]'>
              <span className='font-black text-slate-800 uppercase block tracking-wider text-[9px]'>Cam kết Di Động Việt</span>
              <div className='grid grid-cols-2 gap-2 text-gray-500 font-medium'>
                {[
                  { icon: ShieldCheck, color: 'text-emerald-600', label: 'Bảo hành 12-24T' },
                  { icon: RotateCw, color: 'text-blue-500', label: '1 đổi 1 trong 30 ngày' },
                  { icon: Truck, color: 'text-purple-600', label: 'Giao hỏa tốc 2h' },
                  { icon: BadgeCheck, color: 'text-amber-500', label: 'Trả góp 0% lãi' },
                ].map((item, idx) => (
          ...
          <div className='lg:col-span-7 space-y-4 relative z-10 flex flex-col justify-between'>
                ...
                <div className='flex items-center gap-0.5 text-amber-500'>
                  {[...Array(5)].map((_, i) => (
                    <Star 
                      key={i} 
                      size={12} 
                      className={i < Math.round(product.ratingsAverage || 5) ? 'fill-amber-500 text-amber-500' : 'text-gray-200'} 
                    />
                  ))}
                  <span className='ml-1 font-bold text-slate-800'>{product.ratingsAverage || 5.0}</span>
                  <span className='text-gray-400'>({product.ratingsCount || 0} đánh giá)</span>
                </div>
                
                <span className='h-3 w-px bg-gray-200' />
                <span className='font-mono'>SKU: <strong className='text-slate-800'>{activeVariant.sku || 'N/A'}</strong></span>
              </div>
            </div>
            ...
            <div className='bg-slate-50 border border-slate-100 rounded-xl p-4 space-y-3'>
              <div className='flex items-baseline flex-wrap gap-2'>
                <span className='text-xl sm:text-2xl font-black text-didongviet-red'>{formatVND(salePrice)}</span>
                {percentOff > 0 && (
                  <span className='text-xs font-bold text-gray-400 line-through'>{formatVND(originalPrice)}</span>
                )}
              </div>
                    ...
                    return (
                      ...
                      >
                        <div className='flex items-center justify-between text-[11px] font-bold'>
                          <span className={`${idx === selectedVariantIdx ? 'text-didongviet-red' : 'text-slate-800'}`}>
                            {v.color} - {v.ram}/{v.rom}
                          </span>
                          {idx === selectedVariantIdx && <span className='h-1.5 w-1.5 rounded-full bg-didongviet-red' />}
                        </div>
              ...
              <div className='flex items-center gap-3'>
                ...
                <span className='text-[10px] font-semibold text-gray-500'>
                  {currentVariantStock > 0 ? (
                    <span>Tồn kho: <strong className='text-emerald-600 font-black'>{currentVariantStock} máy</strong></span>
                  ) : (
              ...
              <div className='grid grid-cols-2 gap-3'>
                ...
                >
                  ...
                  <div className='text-left leading-tight'>
                    ...
                    <span className='block text-[8px] font-normal text-white/80'>Hoặc nhận tại cửa hàng</span>
                  </div>
                </Button>
                ...
                <Button 
                  ...
                  onClick={() => setAlert({ type: 'success', message: 'Chuyển hướng trả góp 0%...' })}
                >
                  <span className='font-black'>TRẢ GÓP 0%</span>
                  <span className='text-[8px] font-normal text-slate-500 mt-0.5'>Duyệt online 5 phút</span>
                </Button>
              </div>
            </div>
          </div>
          ...
          <section className='bg-white rounded-2xl p-5 border border-slate-100 shadow-xs space-y-3'>
            ...
            <div className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2.5'>
              {activeVariant.inventory.map((inv: any) => {
                const branchObj = branches.find(b => b._id === (inv.branch?._id || inv.branch));
                if (!branchObj) return null;

                return (
                  <div key={inv.branch} className='p-3 bg-slate-50 rounded-lg border border-slate-100 flex items-center justify-between gap-2 text-[10px]'>
                    <div className='space-y-0.5 truncate'>
                      <span className='font-bold text-slate-800 block truncate' title={branchObj.name}>{branchObj.name}</span>
                      <span className='text-[9px] text-gray-400 block truncate' title={branchObj.address}>{branchObj.address}</span>
                    </div>
                    <span className={`px-1.5 py-0.5 rounded-full font-bold uppercase text-[8px] border flex-shrink-0
                      ${inv.stock > 0 
        ...
        <section className='grid grid-cols-1 lg:grid-cols-12 gap-5 items-start'>
          ...
          <div className='lg:col-span-7 bg-white rounded-2xl p-5 border border-slate-100 shadow-xs space-y-4'>
            <div className='flex items-center gap-2 border-b border-slate-100 pb-3'>
              ...
              <h3 className='text-xs font-black text-slate-800 uppercase tracking-tight'>Bài viết đánh giá chi tiết</h3>
            </div>
            
            <div className={`relative overflow-hidden text-xs text-gray-600 leading-relaxed font-medium transition-all duration-300
              ${isDescExpanded ? 'max-h-none' : 'max-h-[250px]'}
            `}>
              <div className='space-y-3 whitespace-pre-line'>
                {product.description || `Sản phẩm ${product.name} chính hãng hiện đang được bán tại hệ thống Di Động Việt với mức giá vô cùng cạnh tranh. 
          ...
          <div className='lg:col-span-5 bg-white rounded-2xl p-5 border border-slate-100 shadow-xs space-y-3'>
            <div className='flex items-center gap-2 border-b border-slate-100 pb-3'>
              ...
              <h3 className='text-xs font-black text-slate-800 uppercase tracking-tight'>Thông số kỹ thuật</h3>
            </div>
            ...
            <div className='overflow-hidden rounded-lg border border-slate-100 text-[11px]'>
              <table className='w-full border-collapse'>
                <tbody>
                  {[
                    { label: 'Thương hiệu', value: product.brand || 'Di Động Việt' },
                    { label: 'Ngành hàng', value: product.category?.name || 'Sản phẩm' },
                    { label: 'Màu sắc', value: product.variants ? Array.from(new Set(product.variants.map((v: any) => v.color))).join(', ') : 'N/A' },
                    { label: 'RAM', value: product.variants ? Array.from(new Set(product.variants.map((v: any) => v.ram))).join(', ') : 'N/A' },
                    { label: 'ROM', value: product.variants ? Array.from(new Set(product.variants.map((v: any) => v.rom))).join(', ') : 'N/A' },
                    { label: 'Độ mới', value: product.isUsed ? 'Likenew 99%' : 'Nguyên seal 100%' },
                  ].map((row, idx) => (
                    <tr key={idx} className={`${idx % 2 === 0 ? 'bg-slate-50/50' : ''} hover:bg-slate-50 transition-colors`}>
                      <td className='py-2.5 px-3 font-bold text-gray-500 w-1/3 border-b border-slate-100'>{row.label}</td>
                      <td className='py-2.5 px-3 font-semibold text-slate-800 border-b border-slate-100'>{row.value}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
        ...
        {relatedProducts.length > 0 && (
          <section className='space-y-4'>
            <div className='flex items-center gap-2 border-b border-slate-200 pb-3'>
              ...
              <h3 className='text-sm font-black text-slate-800 uppercase tracking-tight'>Sản phẩm tương tự</h3>
            </div>
