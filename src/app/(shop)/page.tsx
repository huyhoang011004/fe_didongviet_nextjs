  ...
  const banners = [
    {
      ...
      subtitle: 'Trợ giá thu cũ lên đời đến 5.000.000đ. Độc quyền D.Member giảm thêm 1%.',
      link: '#',
      badge: 'HOT DEAL'
    },
    {
      ...
      subtitle: 'iPad Pro M4, MacBook Air M3 giảm kịch sàn. Hỗ trợ trả góp 0% lãi suất.',
      link: '#',
      badge: 'APPLE WEEK'
    },
    {
      ...
      subtitle: 'Củ sạc nhanh, cáp sạc, tai nghe bluetooth chính hãng chỉ từ 99K.',
      link: '#',
      badge: 'SALE CHẠM ĐÁY'
    }
  ];
  ...
  useEffect(() => {
    async function loadHomepageData() {
      try {
        ...
        const [categoriesRes, productsRes, tradeInRes, blogsRes] = await Promise.all([
          fetch(`${apiUrl}/categories`).then(r => r.ok ? r.json() : null),
          fetch(`${apiUrl}/products`).then(r => r.ok ? r.json() : null),
          fetch(`${apiUrl}/products/trade-in`).then(r => r.ok ? r.json() : null),
          fetch(`${apiUrl}/blogs?limit=3`).then(r => r.ok ? r.json() : null),
        ]);
        ...
        if (productsRes && productsRes.success) {
          ...
          const sales = prods.filter((p: any) => 
            p.variants && p.variants.some((v: any) => v.salePrice && v.salePrice < v.price)
          );
        ...
        if (tradeInRes && tradeInRes.success) {
          ...
          const fallbackTrade = allProducts.filter((p: any) => p.tradeInBonus > 0);
    ...
    <div className='min-h-screen bg-slate-50 font-sans'>
            ...
            {banners.map((b, idx) => (
              ...
              >
                <div className='max-w-md text-white space-y-2.5'>
                  ...
                  <div className='pt-1'>
                    <Button asChild className='bg-didongviet-red hover:bg-didongviet-dark-red text-white rounded-xl py-2.5 px-5 text-xs font-bold cursor-pointer transition-transform hover:scale-105 border-none shadow-sm'>
          ...
          <div className='lg:col-span-1 flex flex-col gap-3'>
            <div className='flex-1 rounded-2xl border border-dashed border-red-200 bg-red-50/40 p-4 flex flex-col justify-center text-center space-y-2.5 hover:border-red-400 transition-all'>
              ...
              <h3 className='text-sm font-black text-slate-800 leading-tight'>
                Mở Thẻ Thành Viên<br />Nhận Vạn Ưu Đãi
              </h3>
              <p className='text-[10px] text-gray-500 leading-relaxed'>
                Giảm thêm đến 1.5% hóa đơn, tích điểm thưởng và nhận quà sinh nhật.
              </p>
              <div className='pt-0.5'>
                <Button asChild size='sm' className='bg-slate-900 hover:bg-slate-800 text-white border-none rounded-xl text-[11px] font-bold cursor-pointer shadow-xs h-8 px-4'>
            ...
            <div className='grid grid-cols-2 gap-2.5'>
              <div className='rounded-xl border border-slate-100 bg-white p-3 flex flex-col items-center justify-center text-center hover:shadow-sm transition-shadow'>
                ...
                <span className='text-[9px] font-black text-slate-800 uppercase'>Giao nhanh 2H</span>
                <span className='text-[8px] text-gray-400 mt-0.5'>Nội thành HCM - HN</span>
              </div>
              <div className='rounded-xl border border-slate-100 bg-white p-3 flex flex-col items-center justify-center text-center hover:shadow-sm transition-shadow'>
                <ShieldCheck className='text-emerald-500 mb-1' size={18} />
                <span className='text-[9px] font-black text-slate-800 uppercase'>Bảo hành 24T</span>
                <span className='text-[8px] text-gray-400 mt-0.5'>Độc quyền DDV</span>
              </div>
            </div>
          </div>
        </div>
      ...
      <section className='max-w-6xl mx-auto px-4 -mt-1 relative z-10'>
        <div className='grid grid-cols-2 lg:grid-cols-4 gap-3 bg-white rounded-2xl p-4 border border-slate-100 shadow-xs'>
          {[
            { icon: Award, color: 'text-didongviet-red bg-red-50', title: '100% CHÍNH HÃNG', desc: 'Cam kết chính hãng toàn cầu' },
            { icon: RotateCw, color: 'text-blue-500 bg-blue-50', title: '1 ĐỔI 1 TRONG 30 NGÀY', desc: 'Nếu phát sinh lỗi NSX' },
            { icon: Truck, color: 'text-purple-600 bg-purple-50', title: 'GIAO HÀNG MIỄN PHÍ', desc: 'Free ship toàn quốc' },
            { icon: UserCheck, color: 'text-amber-600 bg-amber-50', title: 'CSKH TẬN TÂM', desc: 'Hỗ trợ kỹ thuật 24/7' },
          ].map((item, idx) => (
            <div key={idx} className='flex items-center gap-2.5 p-2'>
              <div className={`h-9 w-9 rounded-lg ${item.color} flex items-center justify-center flex-shrink-0`}>
                <item.icon size={18} />
              </div>
              <div>
                <span className='text-[10px] font-black text-slate-800 uppercase block leading-tight'>{item.title}</span>
      ...
      <div className='max-w-6xl mx-auto px-4 space-y-8 py-8'>
          ...
          <div className='bg-slate-900 rounded-2xl p-5 md:p-6 space-y-4 shadow-lg border border-slate-800 relative overflow-hidden'>
              ...
              <div className='flex items-center gap-2'>
                ...
                <div>
                  <h2 className='text-base sm:text-lg font-black text-white uppercase tracking-tight flex items-center gap-2'>
                    ...
                    <span className='text-[9px] bg-red-600 text-white font-extrabold px-1.5 py-0.5 rounded animate-pulse'>HOT</span>
                  </h2>
                  <p className='text-[10px] text-slate-400 mt-0.5 font-medium'>Săn ngay deal tốt giá kịch sàn!</p>
                </div>
              </div>
                ...
                <div className='flex gap-1 items-center'>
                  {['02', '45', '30'].map((t, i) => (
                    <span key={i} className='flex items-center gap-1'>
                      <span className='px-2 py-1 bg-slate-800 text-white rounded-md border border-slate-700 font-mono text-[10px]'>{t}</span>
                      {i < 2 && <span>:</span>}
                    </span>
                  ))}
                </div>
            ...
            {loading ? (
              ...
              <div className='text-center py-10 text-slate-400 border border-dashed border-slate-800 rounded-xl bg-slate-950/20'>
                <ShoppingBag size={36} className='mx-auto text-slate-700 mb-2' />
                <p className='text-xs font-semibold'>Deal sốc đang được chuẩn bị. Quay lại sau!</p>
              </div>
            ) : (
              <div className='grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3 relative z-10'>
                {flashSaleProducts.map(p => {
                  ...
                  const activeVariant = p.variants?.find((v: any) => v.salePrice && v.salePrice < v.price) || p.variants?.[0];
                  ...
                  return (
                    <div key={p._id} className='bg-slate-950/40 border border-slate-800/80 rounded-xl p-3 flex flex-col justify-between space-y-3 hover:border-didongviet-red/60 hover:shadow-lg transition-all group relative overflow-hidden'>
                      {percentOff > 0 && (
                      ...
                      <div className='space-y-2'>
                        ...
                        <div className='space-y-0.5'>
                          <span className='text-[9px] text-slate-500 font-bold block uppercase'>{p.brand || 'DI ĐỘNG VIỆT'}</span>
                          <Link href={`/product/${p.slug || p._id}`} className='font-bold text-white text-[11px] sm:text-xs hover:text-didongviet-red block truncate leading-snug'>
                            {p.name}
                          </Link>
                          ...
                          <div className='flex items-center gap-1 text-[9px] text-amber-400 font-semibold'>
                            ...
                            <span className='text-slate-500'>({p.ratingsCount || 0})</span>
                          </div>
                        </div>
                      </div>
                      ...
                      <div className='space-y-1.5 pt-1 border-t border-slate-900'>
                        <div className='flex flex-wrap items-baseline gap-1'>
                          ...
                          {percentOff > 0 && (
                            <span className='text-[9px] font-bold text-slate-500 line-through'>{formatVND(originalPrice)}</span>
                          )}
                        </div>
                        ...
                        <div className='space-y-0.5'>
                          <div className='h-1 w-full bg-slate-800 rounded-full overflow-hidden'>
                            <div className='h-full bg-didongviet-red rounded-full animate-pulse' style={{ width: '70%' }} />
                          </div>
                          <span className='text-[8px] font-bold text-slate-400 block'>Đã bán: 18 máy</span>
                        </div>
                      </div>
                    </div>
                  );
          ...
          <div className='bg-gradient-to-r from-blue-50 to-indigo-50 border border-blue-100 rounded-2xl p-5 md:p-6 space-y-4 shadow-xs relative overflow-hidden'>
              ...
              <div className='flex items-center gap-2'>
                ...
                <div>
                  <h2 className='text-base sm:text-lg font-black text-slate-800 uppercase tracking-tight flex items-center gap-2'>
                    ...
                    <span className='text-[9px] bg-blue-600 text-white font-extrabold px-1.5 py-0.5 rounded'>ĐỘC QUYỀN</span>
                  </h2>
                  <p className='text-[10px] text-gray-500 mt-0.5 font-medium'>Lên đời smartphone, tiết kiệm tối đa!</p>
                </div>
              </div>
              
              <Button asChild variant='ghost' size='sm' className='text-[11px] text-blue-600 font-bold hover:bg-blue-50/50 rounded-xl cursor-pointer self-start sm:self-center h-8'>
            ...
            {loading ? (
              ...
              <div className='text-center py-10 text-gray-400 border border-dashed border-gray-200 rounded-xl bg-white'>
                ...
                <p className='text-xs font-semibold'>Chưa có deal thu cũ đổi mới. Quay lại sau!</p>
              </div>
            ) : (
              <div className='grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3 relative z-10'>
                {tradeInProducts.map(p => {
                  ...
                  return (
                    <div key={p._id} className='bg-white border border-blue-100 rounded-xl p-3 flex flex-col justify-between space-y-3 hover:border-blue-400 hover:shadow-md transition-all group relative overflow-hidden'>
                      ...
                      <div className='space-y-2'>
                        ...
                        <div className='space-y-0.5'>
                          ...
                          <Link href={`/product/${p.slug || p._id}`} className='font-bold text-slate-800 text-[11px] sm:text-xs hover:text-blue-600 block truncate leading-snug'>
                            {p.name}
                          </Link>
                      ...
                      <div className='space-y-1 pt-1 border-t border-gray-100'>
                        <span className='text-xs sm:text-sm font-black text-blue-600'>{formatVND(salePrice)}</span>
                        
                        <p className='text-[8px] text-gray-400 font-semibold leading-relaxed'>
                          Sau thu đổi: <strong className='text-didongviet-red text-[10px] font-black'>{formatVND(salePrice - (p.tradeInBonus || 0))}</strong>
                        </p>
                      </div>
                    </div>
                  );
        ...
        <section className='space-y-4'>
          <div className='text-center space-y-0.5'>
            <h2 className='text-base sm:text-lg font-black text-slate-800 uppercase tracking-tight'>DANH MỤC NỔI BẬT</h2>
            <p className='text-[10px] text-gray-400 font-medium'>Tìm kiếm nhanh sản phẩm theo ngành hàng</p>
          </div>

          <div className='grid grid-cols-4 sm:grid-cols-4 md:grid-cols-6 lg:grid-cols-8 gap-3'>
            {categories.slice(0, 8).map((cat, idx) => {
              const icons = [Smartphone, Tablet, Laptop, Tv, Headphones, Award, Zap, RotateCw];
            ...
            {categories.length === 0 && (
              [
                ...
                { name: 'Máy cũ giá rẻ', slug: 'dien-thoai-cu', icon: RotateCw },
        ...
        <section className='space-y-4'>
          <div className='flex items-center justify-between border-b border-slate-200 pb-3'>
            <div className='flex items-center gap-2'>
              ...
              <h2 className='text-sm sm:text-base font-black text-slate-800 uppercase tracking-tight'>SẢN PHẨM KHUYẾN NGHỊ</h2>
            </div>
            
            <Button asChild variant='ghost' size='sm' className='text-[10px] text-didongviet-red font-bold hover:bg-red-50 rounded-xl cursor-pointer h-7'>
          ...
          {loading ? (
            <div className='grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3 animate-pulse'>
              {[...Array(10)].map((_, i) => (
                <div key={i} className='bg-white h-[240px] rounded-xl border border-slate-100' />
              ))}
            </div>
        ...
        <section className='space-y-4'>
          <div className='flex items-center justify-between border-b border-slate-200 pb-3'>
            <div className='flex items-center gap-2'>
              ...
              <h2 className='text-sm sm:text-base font-black text-slate-800 uppercase tracking-tight'>TIN TỨC CÔNG NGHỆ</h2>
            </div>
            
            <Button asChild variant='ghost' size='sm' className='text-[10px] text-purple-600 font-bold hover:bg-purple-50 rounded-xl cursor-pointer h-7'>
          ...
          {loading ? (
            <div className='grid grid-cols-1 md:grid-cols-3 gap-4 animate-pulse'>
              {[...Array(3)].map((_, i) => (
                <div key={i} className='bg-white h-[200px] rounded-xl border border-slate-100' />
              ))}
            </div>
          ) : blogs.length === 0 ? (
            <div className='text-center py-10 text-gray-400 border border-dashed border-gray-200 rounded-xl bg-white'>
              <Calendar size={36} className='mx-auto text-gray-300 mb-2' />
              <p className='text-xs font-semibold'>Chưa có tin công nghệ nào. Quay lại sau!</p>
            </div>
          ) : (
            <div className='grid grid-cols-1 md:grid-cols-3 gap-4'>
              {blogs.map(b => (
                <div key={b._id} className='bg-white border border-slate-100 rounded-xl overflow-hidden shadow-xs hover:shadow-md transition-shadow group flex flex-col justify-between'>
                  <div className='space-y-3'>
                    ...
                    <div className='px-4 space-y-1.5'>
                      ...
                      <Link href={`/blog/${b.slug || b._id}`} className='font-black text-slate-800 text-xs sm:text-sm hover:text-purple-600 block line-clamp-2 leading-snug'>
                        {b.title}
                      </Link>
                  ...
                  <div className='px-4 pb-4 pt-2'>
                    <Link href={`/blog/${b.slug || b._id}`} className='inline-flex items-center gap-1 text-[10px] font-bold text-purple-600 hover:text-purple-700 hover:underline'>
