  ...
  if (loading) {
    return (
      <div className='min-h-screen bg-slate-50 flex flex-col items-center justify-center p-8'>
        ...
        <p className='mt-3 text-xs font-medium text-slate-500 animate-pulse'>Đang tải hồ sơ cá nhân...</p>
      </div>
    );
  }
        ...
        <aside className='lg:col-span-1 space-y-4 h-fit sticky top-20'>
          <div className='bg-white rounded-xl border border-slate-100 p-2 shadow-xs'>
            <button 
              ...
              `}
            >
              <Settings size={16} className={activeTab === 'info' ? 'text-didongviet-red' : 'text-slate-400'} />
            ...
            <button 
              ...
              `}
            >
              <Package size={16} className={activeTab === 'orders' ? 'text-didongviet-red' : 'text-slate-400'} />
              ...
              <div className='bg-white rounded-xl border border-slate-100 shadow-xs p-5'>
                <div className='flex items-center justify-between border-b border-slate-100 pb-3 mb-4'>
                  <div className='flex items-center gap-2'>
                    ...
                    <h2 className='text-sm font-black text-slate-800 uppercase tracking-tight'>Thông tin bảo mật</h2>
                  </div>
                  {!isEditing ? (
                    <Button onClick={() => setIsEditing(true)} variant='outline' size='sm' className='text-[10px] h-7 font-bold'>
                      Cập nhật
                    </Button>
                  ) : (
                    <div className='flex gap-2'>
                      <Button onClick={() => setIsEditing(false)} variant='outline' size='sm' className='text-[10px] h-7 font-bold border-slate-200'>
                        Hủy
                      </Button>
                      <Button onClick={handleSaveProfile} disabled={saving} size='sm' className='bg-didongviet-red hover:bg-didongviet-dark-red text-white text-[10px] h-7 font-bold shadow-xs border-none'>
                ...
                <div className='grid grid-cols-1 md:grid-cols-2 gap-4'>
                  ...
                  <div className='space-y-1.5'>
                    <label className='text-[10px] font-bold text-slate-500 uppercase'>Email (Không thể đổi)</label>
                    <Input 
                      value={user.email}
                      disabled
                  ...
                  <div className='space-y-1.5'>
                    <label className='text-[10px] font-bold text-slate-500 uppercase'>Loại tài khoản</label>
                    <Input 
                      value={user.role === 'admin' ? 'Quản trị viên (Admin)' : user.role === 'staff' ? 'Nhân viên (Staff)' : 'Khách hàng (User)'}
                      disabled
              ...
              {studentProfile && (
                <div className='bg-blue-50/50 rounded-xl border border-blue-100 shadow-xs p-5'>
                  <div className='flex items-center gap-2 border-b border-blue-100/50 pb-3 mb-4'>
                    ...
                    <h2 className='text-sm font-black text-slate-800 uppercase tracking-tight'>Hồ sơ Học sinh - Sinh viên</h2>
                    <span className={`px-2 py-0.5 rounded-full text-[9px] font-bold border ${studentProfile.isHSSVVerified === 'Đã xác thực' ? 'bg-emerald-50 text-emerald-600 border-emerald-200' : 'bg-amber-50 text-amber-600 border-amber-200'}`}>
                  ...
                  <div className='grid grid-cols-1 sm:grid-cols-3 gap-4'>
                    <div className='space-y-1'>
                      <span className='text-[10px] font-bold text-slate-500 uppercase block'>Trường học</span>
                      <span className='text-[11px] font-bold text-slate-800 block truncate'>{studentProfile.schoolName}</span>
                    </div>
                    <div className='space-y-1'>
                      <span className='text-[10px] font-bold text-slate-500 uppercase block'>MSSV</span>
                      <span className='text-[11px] font-bold text-slate-800 block'>{studentProfile.studentId}</span>
                    </div>
                    <div className='space-y-1'>
                      <span className='text-[10px] font-bold text-slate-500 uppercase block'>Ngày tốt nghiệp dự kiến</span>
          ...
          {activeTab === 'orders' && (
            <div className='bg-white rounded-xl border border-slate-100 shadow-xs p-5 space-y-4'>
              <div className='flex items-center gap-2 border-b border-slate-100 pb-3 mb-2'>
                ...
                <h2 className='text-sm font-black text-slate-800 uppercase tracking-tight'>Lịch sử mua hàng</h2>
              </div>

              {!user.orderHistory || user.orderHistory.length === 0 ? (
                <div className='text-center py-10 bg-slate-50 rounded-xl border border-dashed border-slate-200'>
                  <Package size={32} className='mx-auto text-slate-300 mb-2' />
                  <p className='text-xs font-semibold text-slate-500'>Bạn chưa có đơn hàng nào.</p>
                  <Button asChild className='mt-3 bg-didongviet-red hover:bg-didongviet-dark-red text-white h-8 text-[10px] font-bold rounded-lg border-none'>
                ...
                <div className='space-y-3'>
                  {user.orderHistory.map((order: any, idx: number) => (
                    <div key={order._id || idx} className='p-4 border border-slate-100 rounded-xl hover:shadow-sm transition-all'>
                      <div className='flex flex-wrap items-center justify-between gap-2 border-b border-slate-50 pb-2 mb-2 text-[10px] font-bold text-slate-500'>
                        <span className='font-mono'>Mã ĐH: <strong className='text-slate-800'>{order.orderNumber || order._id?.slice(-8).toUpperCase()}</strong></span>
                        <span className='px-1.5 py-0.5 rounded bg-blue-50 text-blue-600 border border-blue-100 uppercase'>{order.orderStatus || 'Đang xử lý'}</span>
                      </div>
                      <div className='flex items-center justify-between'>
                        <span className='text-[11px] font-bold text-slate-700'>{new Date(order.createdAt).toLocaleDateString('vi-VN')}</span>
                        <span className='text-xs font-black text-didongviet-red'>{formatVND(order.totalPrice || 0)}</span>
                      </div>
                    </div>
                  ))}
                </div>
