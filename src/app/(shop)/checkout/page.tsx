...
export default function CheckoutPage() {
    ...
    async function loadData() {
      try {
        ...
        if (profileRes.success && profileRes.data) {
          ...
          const defaultAddress = user.address?.find((a: any) => a.isDefault) || user.address?.[0];
          if (defaultAddress) {
  ...
  const handlePlaceOrder = async () => {
    if (!fullName || !phone || !province || !district || !ward || !streetAddress) {
      setAlert({ type: 'error', message: 'Vui lòng điền đầy đủ thông tin giao hàng!' });
      return;
    }
    ...
    try {
      ...
      if (data.success) {
        setAlert({ type: 'success', message: 'Đặt hàng thành công! Đang chuyển hướng...' });
        setTimeout(() => {
  ...
  if (loading) {
    return (
      <div className='min-h-screen bg-slate-50 flex flex-col items-center justify-center p-8'>
        ...
        <p className='mt-3 text-xs font-medium text-slate-500 animate-pulse'>Đang chuẩn bị trang thanh toán...</p>
      </div>
    );
  }
    ...
    <div className='min-h-screen bg-slate-50 font-sans text-slate-700 pb-12'>
      ...
      <nav className='bg-white border-b border-slate-100 py-2.5'>
        <div className='max-w-6xl mx-auto px-4 flex items-center gap-1.5 text-[10px] text-gray-400 font-semibold'>
          ...
          <Link href='/cart' className='hover:text-didongviet-red transition-colors'>Giỏ hàng</Link>
          <ChevronRight size={10} />
      ...
      <div className='max-w-6xl mx-auto px-4 py-6'>
        ...
        <div className='flex items-center gap-2.5 mb-5'>
          ...
          <div>
            <h1 className='text-base sm:text-lg font-black text-slate-800 uppercase tracking-tight'>Thanh toán an toàn</h1>
            <p className='text-[10px] text-slate-400 font-medium'>Vui lòng kiểm tra lại thông tin giao hàng</p>
          </div>
        </div>
        ...
        <div className='grid grid-cols-1 lg:grid-cols-12 gap-5'>
              ...
              <div className='grid grid-cols-1 md:grid-cols-2 gap-4'>
                <div className='space-y-1.5'>
                  ...
                  <Input 
                    ...
                    placeholder='Nhập họ tên người nhận' className='text-xs h-10' 
                  />
                </div>
                <div className='space-y-1.5'>
                  ...
                  <Input 
                    ...
                    placeholder='Nhập số điện thoại' className='text-xs h-10' 
                  />
                </div>
              </div>
            </div>
              ...
              <div className='grid grid-cols-1 md:grid-cols-3 gap-4'>
                <div className='space-y-1.5'>
                  ...
                  <Input 
                    ...
                    placeholder='VD: TP. Hồ Chí Minh' className='text-xs h-10' 
                  />
                </div>
                <div className='space-y-1.5'>
                  ...
                  <Input 
                    ...
                    placeholder='VD: Quận 1' className='text-xs h-10' 
                  />
                </div>
                <div className='space-y-1.5'>
                  ...
                  <Input 
                    ...
                    placeholder='VD: Phường Bến Nghé' className='text-xs h-10' 
                  />
                </div>
              </div>
              <div className='space-y-1.5'>
                <label className='text-[10px] font-bold text-slate-500 uppercase'>Số nhà, tên đường *</label>
                <Input 
                  value={streetAddress} onChange={(e) => setStreetAddress(e.target.value)} 
                  placeholder='VD: 75 Nguyễn Bỉnh Khiêm' className='text-xs h-10' 
                />
              </div>
            </div>
            ...
            <div className='bg-white rounded-xl border border-slate-100 shadow-xs p-5 space-y-4'>
              ...
              <div className='grid grid-cols-1 gap-3'>
                <label className='relative flex items-center gap-3 p-3 rounded-xl border-2 border-didongviet-red bg-red-50/50 cursor-pointer transition-all'>
                  <input type='radio' name='payment' defaultChecked className='h-4 w-4 text-didongviet-red accent-didongviet-red' />
                  <div className='flex flex-col'>
                    <span className='text-xs font-bold text-slate-800'>Thanh toán khi nhận hàng (COD)</span>
                    <span className='text-[10px] text-slate-500'>Kiểm tra hàng trước khi thanh toán.</span>
                  </div>
                </label>
                ...
                <label className='relative flex items-center gap-3 p-3 rounded-xl border border-slate-200 opacity-60 cursor-not-allowed'>
                  ...
                  <div className='flex flex-col'>
                    <span className='text-xs font-bold text-slate-800'>Thanh toán online (VNPAY / MOMO)</span>
                    <span className='text-[10px] text-slate-500'>Tính năng đang được bảo trì.</span>
                  </div>
                </label>
              </div>
            </div>
          ...
          <div className='lg:col-span-4 space-y-4'>
            ...
            <div className='bg-white rounded-xl border border-slate-100 shadow-xs p-5 space-y-4'>
              ...
              <div className='space-y-3 max-h-60 overflow-y-auto pr-2 custom-scrollbar'>
                {cart.items.map((item: any, idx: number) => {
                  ...
                  const thumbUrl = typeof item.product === 'object' 
                    ? (item.product.images?.find((img: any) => img.isThumbnail)?.url || item.product.images?.[0]?.url || '/placeholder-product.png')
                  ...
                  return (
                    <div key={idx} className='flex gap-3'>
                      <div className='h-12 w-12 rounded bg-slate-50 border border-slate-100 flex-shrink-0 flex items-center justify-center p-1'>
                        <img src={thumbUrl} alt='thumb' className='h-full w-full object-contain' />
                      </div>
                      <div className='flex-1 min-w-0'>
                        <h4 className='text-[11px] font-bold text-slate-800 truncate'>{productName}</h4>
                        <div className='flex items-center justify-between mt-1'>
                          <span className='text-[10px] text-slate-500 font-semibold'>SL: {item.quantity}</span>
                          <span className='text-[11px] font-black text-didongviet-red'>{formatVND(item.price)}</span>
                        </div>
                      </div>
                    </div>
                  );
              ...
              <div className='border-t border-slate-100 pt-3 space-y-2 text-[11px]'>
                <div className='flex items-center justify-between text-slate-500 font-medium'>
                  <span>Tạm tính</span>
                  <span className='font-bold text-slate-700'>{formatVND(cart.totalPrice)}</span>
                </div>
                ...
                <div className='flex items-center justify-between text-slate-500 font-medium'>
                  ...
                  <span className={shippingPrice === 0 ? 'font-bold text-emerald-600' : 'font-bold text-slate-700'}>
              ...
              <div className='border-t border-slate-100 pt-3'>
                <div className='flex items-center justify-between mb-4'>
                  ...
                  <span className='text-xl font-black text-didongviet-red'>{formatVND(grandTotal)}</span>
                </div>
            ...
            <div className='bg-white rounded-xl border border-slate-100 shadow-xs p-4 flex items-start gap-3'>
              ...
              <div>
                <h4 className='text-[11px] font-bold text-slate-800 uppercase'>Giao hàng siêu tốc 2H</h4>
                <p className='text-[10px] text-slate-500 mt-0.5'>Nhận hàng trong vòng 2 giờ tại khu vực nội thành. Đảm bảo an toàn, nhanh chóng.</p>
              </div>
            </div>
